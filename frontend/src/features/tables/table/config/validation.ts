...
const COLUMN_TYPE_VALUES = ['string', 'number', 'timestamp', 'enum'] as const
    ...
    [COLUMN_FIELD_NAME]: array().of(
        object({
            ...
            type: mixed<'string' | 'number' | 'timestamp' | 'enum'>()
                .oneOf(COLUMN_TYPE_VALUES, 'Недопустимый тип столбца')
            ...
            enum: array()
                ...
                .when('type', {
                    is: 'enum',
                    then: (s) => s.min(1, 'Добавьте хотя бы одно значение enum').required('Укажите значения enum'),
                    otherwise: (s) => s.strip(),
                }),
